...
for kT in kT_vals:
    ref_fname="piongrid_bbref_kT%s.qdp" % str(kT)
    ref_data=np.loadtxt(ref_fname,skiprows=1)
    for xi in logxi_vals:
        for col_density in col_density_vals:
            for v in v_vals:
                ...
                spec_fname="piongrid_abs_%s.qdp" % modstr
                ...
                spec_array.append(temp_data[:,3]/ref_data[:,3])
